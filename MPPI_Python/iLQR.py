...
def cost(x, u, Q, R):
    ...
    return 0.5 * (x_flat.T @ Q @ x_flat + u.T @ R @ u)

def final_cost(x, xd, Qf):
    x_flat = np.array(x)
    xd_flat = np.array(xd)

    return 0.5 * ((x_flat - xd_flat) @ Qf @ (x_flat - xd_flat).T)
...
xtraj: trajectory (12 x steps)
utraj: control input vector (4 x steps-1)
vtraj: offset (4 x steps)
Ktraj: gain   (4 x 12 x steps)

output: base utraj (4 x steps-1)
...
def iLQR(x0, xd, steps, dt, Q, R, Qf):
    ...
    nx = len(x0)
    nu = 4
    vtraj = np.zeros((nu, steps))
    Ktraj = np.zeros((nu, nx, steps))
    xtraj = np.zeros((nx, steps))
    utraj = np.zeros((nu, steps-1))
    for i in range(1000):
        print("Iteration: ", i)
...
def main():
    ...
    x0[2] = 2.0  # Start at z = 2m
    xd = np.zeros((nx,1))  # Hover at origin

    # Cost matrices
    Q = np.diag([10, 10, 100, 1, 1, 10, 1, 1, 1, 1, 1, 1])
    R = np.eye(nu) * 0.1
    Qf = np.diag([100, 100, 500, 10, 10, 100, 10, 10, 10, 10, 10, 10])

    # Run iLQR
    xtraj, utraj, _ = iLQR(x0, xd, steps, dt, Q, R, Qf)
    ...
    for i in range(3):
        plt.plot(time, xtraj[i, :], label=labels[i])
