...
position_tolerance_horizontal = 10;     % meters for hover phases, will adjust for landing
position_tolerance_vertical = 5;       % meters
angle_tolerance = 0.6;                 % radians (~45 degrees)
velocity_tolerance = 15;                % m/s
angular_velocity_tolerance = 5;         % rad/s

fprintf('=== MPPI VTOL Control with Quaternion Dynamics and Multi-Phase Trajectory ===\n');
...
function J = runningCost(x, xd_current, R, u, du, nu, phase)
    ...
    if phase < 3
        % Hover phases: Balanced position weights
        ...
        % Landing phase: Relax horizontal, emphasize vertical and orientation
...
function J = finalCost(xT, xd_current, phase)
    ...
    if phase < 3
        % Hover phases: Balanced weights, no altitude penalty
        ...
        % Landing phase: Relax horizontal, high on vertical, orientation, velocities; altitude penalty
